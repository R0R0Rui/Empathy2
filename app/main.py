...
from app.control import apply_runtime_control
...
async def chat(request: ChatRequest) -> ChatResponse:
    ...
    if crisis_detected:
        ...
        try:
            anchor_reply = await asyncio.to_thread(generator.generate, messages)
        except Exception as exc:
    ...
    control_result = apply_runtime_control(
        user_message=request.message,
        anchor_reply=anchor_reply,
        scenario_type=(
            understanding_payload.scenario_tier
            if understanding_payload is not None
            else None
        ),
    ...
    reply = control_result.final_reply
    ...
    return ChatResponse(
        ...
        reply=reply,
        anchor_reply=control_result.anchor_reply,
        raw_reply=control_result.anchor_reply,
        emotions=emotions,
        performance={
            ...
            "control_ms": round(control_ms, 2),
        },
        safety={"crisis_detected": crisis_detected},
        understanding=understanding_payload,
        support_plan=control_result.support_plan,
        validation=control_result.validation,
        refined_validation=control_result.refined_validation,
        refined=control_result.refined,
        refinement_reason=control_result.refinement_reason,
        failure_types=control_result.failure_types,
    )
